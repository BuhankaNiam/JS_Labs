 ...
 * map создаёт новый масив
 */
function getUniqueTransactionTypes(transactions) {
    return [...new Set(transactions.map(t => t.transaction_type))]; /** set()- хранит уникальные значения */
}
...
function calculateTotalAmount(transactions) {
    return transactions.reduce((sum, t) => sum + t.transaction_amount, 0); /** reduce метод массва который сворачивает массив в одно значение */
}

/**
 * Считает сумму по дате (год/месяц/день)
 */
    ...
    return transactions
        .filter(t => {
            ...
            return (!year || d.getFullYear() === year) &&
                (!month || d.getMonth() + 1 === month) &&  /** шоб было 1-12 пишем +1*/
                (!day || d.getDate() === day);
        })
...
function getTransactionByType(transactions, type) {
    return transactions.filter(t => t.transaction_type === type); /**Сравнивает t.transaction_type с переданным type*/
}
/**возвращает масив транзакций совершённых между двумя датами*/
function getTransactionsInDateRange(transactions, startDate, endDate) {
    return transactions.filter(t => {/**filter оставляет только транзакции не раньше и не позже*/
        const d = new Date(t.transaction_date);
        return d >= new Date(startDate) && d <= new Date(endDate);
    });
}
/**возвращает все транзакции, совершённые у конкретного магазина или сервиса*/
function getTransactionsByMerchant(transactions, name) {
    return transactions.filter(t => t.merchant_name === name);
}
/**вычисляет среднюю сумму всех транзакций.*/
function calculateAverageTransactionAmount(transactions) {
    if (!transactions.length) return 0;
    return calculateTotalAmount(transactions) / transactions.length; /**Делим общую сумму на количество транзакций, получая среднее значение*/
}
...
function findMostTransactionsMonth(transactions) {
    const count = {}; /**ключ → номер месяца
 значение → сколько транзакций в этом месяце*/
...
/**Находит месяц, в котором было больше всего транзакций.debit*/
...
/**Находит месяц, в котором было больше всего транзакций по типу*/
...
/**все транзакции, которые были ДО указанной даты.*/
function getTransactionsBeforeDate(transactions, date) {
    return transactions.filter(t =>
        new Date(t.transaction_date) < new Date(date)
    );
}
/**Находит одну транзакцию по её ID*/
function findTransactionById(transactions, id) {
    return transactions.find(t => t.transaction_id === id);
}
/**Возвращает новый массив, в котором есть только описания транзакций.*/
